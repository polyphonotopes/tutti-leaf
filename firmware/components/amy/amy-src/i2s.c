...
// Use the common DAC path: 16-bit Philips I2S. AMY's samples are natively
// 16-bit, and the Sonocotta Amped-ESP32-Plus PCM5122 expects Philips I2S.
// PCM1808 audio-input setups can opt back into I2S_32BIT when needed.
// #define I2S_32BIT
#ifdef I2S_32BIT
...
size_t amy_i2s_write(const uint8_t *buffer, size_t nbytes) {
    ...
    // Optional ESP32 bring-up telemetry: confirm what AMY is handing to the
    // hardware, independently for the interleaved left and right channels.
#ifdef AMY_I2S_DIAGNOSTICS
    static int32_t peak_l = 0;
    static int32_t peak_r = 0;
    static int64_t last_peak_report_us = 0;
    for (int i = 0; i < AMY_BLOCK_SIZE; ++i) {
        int32_t left = block[(i * AMY_NCHANS)];
        int32_t right = block[(i * AMY_NCHANS) + 1];
        if (left < 0) left = -left;
        if (right < 0) right = -right;
        if (left > peak_l) peak_l = left;
        if (right > peak_r) peak_r = right;
    }
    int64_t now_us = amy_get_us();
    if (now_us - last_peak_report_us >= 1000000) {
        fprintf(stderr, "AMY_I2S peak_l=%ld peak_r=%ld bytes=%u\n",
                (long)peak_l, (long)peak_r, (unsigned)nbytes);
        fflush(stderr);
        peak_l = 0;
        peak_r = 0;
        last_peak_report_us = now_us;
    }
#endif
